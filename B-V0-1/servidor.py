...
def handle_client(conn, addr):
    if listen:
        ...
        send(conn, 'Erro: O jogo já começou!')
        send(conn, 'refused_connection')
        conn.close()

def confirm_start(conn):
    global nConfimations
    global dim
    while True:
        send(conn, "your_turn")
        send(conn, 'Deseja começar o jogo [s/n]: ')
        resposta = receive(conn)
        if resposta == 's':
            nConfimations = nConfimations + 1
            send(conn, 'Esperando todos os jogadores confirmarem...\n')
            send(conn, "end_of_your_turn")
            break
        elif resposta == 'n':
            send(conn, "end_of_your_turn")
        else:
            send(conn, 'Resposta invalida.')
            send(conn, "end_of_your_turn")
...
def start_game():
    ...
    global dim
    while True:
        if nConfimations == len(connections) and nConfimations >= 2:
            time.sleep(2)
